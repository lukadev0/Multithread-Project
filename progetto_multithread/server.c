...
void view_my_reservations(int client_socket, const char *username) {
    ...
    if (rc != SQLITE_OK) {
        ...
        if (strlen(buffer) == 0) {
            snprintf(buffer, sizeof(buffer), "Nessuna prenotazione trovata.\n");
        }
...
void cancel_reservation(int client_socket, int reservation_id) {
    ...
    // Aggiorna la disponibilità del libro correlato (se necessario)
    // In questo esempio, assumiamo che si aggiorni la disponibilità di un libro associato alla prenotazione

    // Esempio di aggiornamento della disponibilità del libro:
     char *sql_update_availability = "UPDATE libro SET disponibilita = disponibilita + 1 WHERE id = (SELECT id_libro FROM prenotazione WHERE id = ?);";
